...
Reads a project-root `.env` at startup (gitignored) for API keys used by
the agentic sources: OPENFIGI_API_KEY, FINNHUB_API_KEY, ANTHROPIC_API_KEY.
Docker compose substitutes these into the container env from the same
file; python-dotenv covers the local-uvicorn case.
"""
...
# Load .env BEFORE any module reads its env var.
load_dotenv()
