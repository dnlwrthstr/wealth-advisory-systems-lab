...
### Fact-sheet enrichment (per-fund, LLM-assisted)

For the field groups Yahoo doesn't carry — TER + management/entry/exit fees beyond TER, SRRI/SRI, PRIIPS transaction costs, dealing terms (settlement, cutoff, minimum investment), service providers (depositary / administrator / transfer agent / auditor), benchmark, replication method, rebalance frequency — there's a Claude Code skill at `.claude/skills/find-and-parse-factsheet/SKILL.md`. Per-fund, on-demand, review-then-apply: the skill locates the KID/KIID/fact-sheet PDF for a given ISIN, parses the regulated fields, writes a JSON patch to `data/opensearch/golden/fund/patches/<goldenId>.json`, and only PATCHes the OpenSearch doc after the user confirms. Quality gates in the skill body refuse to apply patches drawn from a wrong-share-class KID or one older than 18 months.
