...
- **fund** — `fund_firds` (FIRDS by ISIN + umbrella LEI map at `src/pipeline/gold/data/fund_umbrellas.yml`) → `fund_yahoo` (NAV / market price / OHLCV / AUM / TER / asset-allocation buckets, no holdings) → `fund_factsheet_patch` (pre-curated patches at `data/opensearch/golden/fund/patches/FG-{ISIN}-001.json`) → `fund_factsheet_skill` (live PDF KID parsing) → `fund_lookthrough_skill` (proxy-derived holdings for synthetic / swap-based ETFs — copies the constituent list from a physically-replicated peer in `pms_golden_fund` tracking the same benchmark; stamps each row `source: physical_proxy` and writes a top-level `lookthroughProvenance` block with proxy ISIN, benchmark, asOfDate, confidence). Cost-class control: CLI defaults `max_cost_class="web_fetch"` (no LLM); `--enable-llm-skills` lifts the cap to `llm_skill`; optional `--llm-skills <id1,id2,...>` restricts which LLM skills run. The deprecated `--enable-factsheet-skill` alias still works for one release.
...
- `fund_firds` — **library only** since spec 003-agentic-fund-universe. Exposes `fetch_by_isin(isin)`, `firds_to_golden`, `load_issuers`, `dedupe_by_isin`, and `derive_*` helpers for the agentic `fund_firds` source to call. The former umbrella-driven bulk CLI moved to `pipeline.agentic.cli.fund_universe`, which routes every share-class ISIN through the full 5-source agentic chain (fund_firds → fund_yahoo → fund_factsheet_patch → fund_factsheet_skill → fund_lookthrough_skill) and persists via `assemble_and_persist` with `universe_status="in_universe"`. Cost-class control: CLI defaults `max_cost_class="web_fetch"` (no LLM); `--enable-llm-skills` lifts the cap to `llm_skill` and makes both LLM-skill sources eligible; `--llm-skills <id1,id2,...>` restricts to specific ones; `--enable-factsheet-skill` is a deprecated alias.
...
**Synthetic-ETF proxy fallback (spec-003).** For swap-based / synthetic ETFs (`replicationMethod=synthetic_swap`), the issuer's own holdings page returns the substitute basket — counterparty collateral, not the economic exposure. `fund_lookthrough_skill` (5th source in the fund chain) handles this case: it queries `pms_golden_fund` for a physically-replicated peer tracking the same benchmark, asks the LLM to rank ≤10 candidates, deep-copies the best match's `assetAllocation.holdings`, stamps every row `source: physical_proxy`, and writes a top-level `lookthroughProvenance` block (proxyIsin, proxyGoldenId, benchmark, asOfDate, confidence). Confidence is computed deterministically post-hoc: `high` on exact `benchmarkIdentifier` match, `medium` on case-insensitive `benchmarkName` match, `low` otherwise. Risk consumers filter proxy-derived rows via `assetAllocation.holdings[].source == "physical_proxy"`; auditors trace lineage via `lookthroughProvenance.*`. Pre-curated patches under `data/opensearch/golden/fund/patches/lookthrough/FG-{ISIN}-001.json` bypass the LLM entirely (`{doc, _meta}` wrapper, same shape as factsheet patches).
