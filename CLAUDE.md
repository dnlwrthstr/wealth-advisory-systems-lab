...
- `issuer_aggregate` — scans `pms_golden_{equity,bond,fund}` via the OpenSearch helpers scan API, dedupes by `issuerId` (`ISS-<LEI>` when available), counts instruments per class, and writes one `IssuerGolden` document per distinct issuer to `pms_golden_issuer`. The embedded `IssuerSnapshot` blocks on instruments are denormalised projections of these canonical records. Re-run after re-loading any per-instrument index.
