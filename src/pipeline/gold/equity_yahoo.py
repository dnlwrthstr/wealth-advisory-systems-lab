"""Yahoo Finance equity fetch — library for the agentic chain.

`fetch_by_identifier(kind, value)` looks up a single equity by ISIN or
ticker via yfinance and returns a validated `EquityGolden` pydantic
instance (or None on miss). `yahoo_info_to_golden` is the core
projection from `Ticker.info` to the golden record.

This module used to host a `--universe` batch CLI that scraped
Wikipedia for SMI / S&P 500 / etc. and bulk-loaded NDJSON. That entry
point has moved to `pipeline.agentic.cli.equity_universe`, which calls
into the full agentic chain (openfigi → equity_firds → equity_yahoo)
per ISIN. The Yahoo fetch retained here is what
`pipeline.agentic.sources.equity_yahoo` calls.
...
regulatory flags and credit/ESG profiles are left to other sources.
"""
